...
serverip()
{
    ...
    SERVER_IP[1]=$(sed -n "/<\/serverip>/p" "./$CONFIG_FILE" | sed -n "s/<serverip>//p" | sed -n "s/<\/serverip>//p" | xargs)
...
serverport()
{
    ...
    SERVER_PORT[1]=$(sed -n "/<\/serverport>/p" "./$CONFIG_FILE" | sed -n "s/<serverport>//p" | sed -n "s/<\/serverport>//p" | xargs)
...
httpport()
{
    ...
    HTTP_PORT[1]=$(sed -n "/<\/httpport>/p" "./$CONFIG_FILE" | sed -n "s/<httpport>//p" | sed -n "s/<\/httpport>//p" | xargs)
...
maxplayers()
{
    ...
    MAX_PLAYERS[1]=$(sed -n "/<\/maxplayers>/p" "./$CONFIG_FILE" | sed -n "s/<maxplayers>//p" | sed -n "s/<\/maxplayers>//p" | xargs)
    ...
    if [ "${MAX_PLAYERS[1]}" -gt "${MAX_PLAYERS[0]}" ] 2>/dev/null;
    then
        # REMOVE SUBSTITUTE LINE
        ...
        sed -i "${LINE} cCHANGE-ME" "./$CONFIG_FILE"
        sed -i -r "${LINE} s|CHANGE-ME|    <maxplayers>${MAX_PLAYERS[0]}</maxplayers>|" "./$CONFIG_FILE"
...
start_server()
{
    # START SERVER COMMAND - MTA 1.6.0+ has native ARM64 support
    echo -e "${YELLOW}Starting MTA Server on ${ARCHITECTURE} architecture..."
    
    # Use mta-server64 for both x86_64 and aarch64 (ARM64)
    if [ -f "./mta-server64" ]; then
        ./mta-server64 -n
    elif [ -f "$START_FILE" ]; then
        "$START_FILE" -n
    else
        echo -e "${YELLOW}ERROR: MTA server binary not found!"
        exit 1
    fi
}
